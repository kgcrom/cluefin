...
def _display_technical_indicators(indicators):
    ...
    if "cup_pattern" in indicators:
        cup_value = indicators["cup_pattern"].iloc[-1] if not pd.isna(indicators["cup_pattern"].iloc[-1]) else 0
        has_volume = indicators.get("cup_has_volume", pd.Series([False])).iloc[-1]

        if len(indicators) < 120:
            signal = "[yellow]⚠️ Insufficient data (need 120+ days)[/yellow]"
            table.add_row("Cup & Handle", "N/A", signal)
        elif cup_value == 100:
            volume_indicator = "✓ Volume confirmed" if has_volume else "⚠️ Price-only"
            signal = f"[bold green]✓ Breakout detected[/bold green]\n[dim]{volume_indicator}[/dim]"
            table.add_row("Cup & Handle", "100", signal)
        else:
            signal = "[dim]No pattern detected[/dim]"
            table.add_row("Cup & Handle", "0", signal)

    # Pattern Recognition: Dow Theory Trend
    if "dow_trend" in indicators:
        trend_value = indicators["dow_trend"].iloc[-1]
        has_volume = indicators.get("dow_has_volume", pd.Series([False])).iloc[-1]

        if len(indicators) < 200:
            signal = "[yellow]⚠️ Insufficient data (need 200+ days)[/yellow]"
            table.add_row("Dow Theory Trend", "N/A", signal)
        elif pd.isna(trend_value):
            signal = "[dim]Insufficient data[/dim]"
            table.add_row("Dow Theory Trend", "N/A", signal)
        else:
            trend_map = {
                2: ("Strong Bull", "green", "✓ Volume confirmed uptrend"),
                1: ("Weak Bull", "yellow", "↗ Uptrend without volume confirmation"),
                0: ("Sideways", "white", "↔ No clear trend"),
                -1: ("Weak Bear", "yellow", "↘ Downtrend without volume confirmation"),
                -2: ("Strong Bear", "red", "✗ Volume confirmed downtrend"),
            }

            trend_name, trend_color, trend_desc = trend_map.get(int(trend_value), ("Unknown", "white", ""))

            # Add volume fallback warning if needed
            if not has_volume:
                trend_desc = f"{trend_desc.split(' (')[0]} (price-only analysis)"

            value_str = f"{int(trend_value):+d} ({trend_name})"
            signal_str = f"[{trend_color}]{trend_desc}[/{trend_color}]"

            # Add index correlation if available
            if "dow_correlation" in indicators:
                corr_value = indicators["dow_correlation"].iloc[-1]
                if not pd.isna(corr_value):
                    if corr_value > 0:
                        signal_str += "\n[dim green]✓ Index confirms[/dim green]"
                    elif corr_value < 0:
                        signal_str += "\n[dim red]⚠ Index diverges[/dim red]"
                    else:
                        signal_str += "\n[dim]○ Neutral correlation[/dim]"

            table.add_row("Dow Theory Trend", value_str, signal_str)
