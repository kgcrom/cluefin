...
@rpc_method(
    ...
    parameters={
        ...
        "properties": {
            ...
            # 서버가 빈 문자열을 거부하므로(1511) trde_qty_tp/trde_prica_tp가 0이어도 전체 범위값을 보낸다.
            "min_trde_qty": {"type": "string", "description": "Min volume (when trde_qty_tp=1). Default 0."},
            "max_trde_qty": {"type": "string", "description": "Max volume (when trde_qty_tp=1). Default 100000000."},
            "min_trde_prica": {
                "type": "string",
                "description": "Min trade amount in million KRW (when trde_prica_tp=1). Default 0.",
            },
            "max_trde_prica": {
                "type": "string",
                "description": "Max trade amount in million KRW (when trde_prica_tp=1). Default 100000000.",
            },
            "stock_code": {
                "type": "string",
...
def handle_kiwoom_volatility_control_event(params: dict, session) -> dict:
    ...
    response = kiwoom.stock_info.get_volatility_control_event(
        ...
        stex_tp=params.get("exchange_type", "1"),
        min_trde_qty=params.get("min_trde_qty", "0"),
        max_trde_qty=params.get("max_trde_qty", "100000000"),
        min_trde_prica=params.get("min_trde_prica", "0"),
        max_trde_prica=params.get("max_trde_prica", "100000000"),
...
def register_kiwoom_stock_info_handlers(dispatcher: DispatcherProtocol) -> None:
