...
class KiwoomRouterAgent:
    ...
    def _classify_intent(self, state: RouterState) -> RouterState:
        ...
        try:
            if self.verbose:
                logger.info(f"Classifying intent for: {state['user_prompt']}")
            ...
            if self.verbose:
                logger.info(
                    f"Classified as: {classification.agent_type.value} (confidence: {classification.confidence})"
                )
    ...
    def _route_to_agent(self, state: RouterState) -> RouterState:
        ...
        try:
            ...
            response = agent.process_request(state["user_prompt"], classification.extracted_params)
    ...
    def _handle_error(self, state: RouterState) -> RouterState:
        ...
        """
        if self.verbose:
            logger.error(f"Error occurred: {state.get('error', 'Unknown error')}")
