...
class BuyTradableAmountItem(BaseModel):
    tr_crcy_cd: Optional[str] = Field(
        title="거래통화코드",
        max_length=3
    )
    ord_psbl_frcr_amt: Optional[str] = Field(
        title="주문가능외화금액",
        max_length=21,
        description="18.2"
    )
    ...
    ord_psbl_qty: Optional[str] = Field(
        title="주문가능수량",
        max_length=10,
        description="22(20.1)"
    )
    exrt: Optional[str] = Field(
        title="환율",
        max_length=22,
        description="25(18.6)"
    )
...
class StockNotConclusionHistoryItem(BaseModel):
    ...
    ord_gno_brno: str = Field(title="주문채번지점번호", max_length=5, description="계좌 개설 시 관리점으로 선택한 영업점의 고유번호")
    ...
    rjct_rson_name: str = Field(title="거부사유명", max_length=60, description="정상 처리되지 못하고 거부된 주문의 사유명")
    ord_tmd: str = Field(title="주문시각", max_length=6, description="주문 접수 시간")
    tr_mket_name: str = Field(title="거래시장명", max_length=60)
    tr_crcy_cd: str = Field(title="거래통화코드", max_length=3, description="USD : 미국달러\nHKD : 홍콩달러\nCNY : 중국위안화\nJPY : 일본엔화\nVND : 베트남동")
    ...
    ovrs_excg_cd: str = Field(title="해외거래소코드", max_length=4, description="NASD : 나스닥\nNYSE : 뉴욕\nAMEX : 아멕스\nSEHK : 홍콩\nSHAA : 중국상해\nSZAA : 중국심천\nTKSE : 일본\nHASE : 베트남 하노이\nVNSE : 베트남 호치민")
    prcs_stat_name: str = Field(title="처리상태명", max_length=60, description='"" 공백 입력')
    loan_type_cd: str = Field(title="대출유형코드", max_length=2, description="00 해당사항없음\n01 자기융자일반형\n03 자기융자투자형\n05 유통융자일반형\n06 유통융자투자형\n07 자기대주\n09 유통대주\n10 현금\n11 주식담보대출\n12 수익증권담보대출\n13 ELS담보대출\n14 채권담보대출\n15 해외주식담보대출\n16 기업신용공여\n31 소액자동담보대출\n41 매도담보대출\n42 환매자금대출\n43 매입환매자금대출\n44 대여매도담보대출\n81 대차거래\n82 법인CMA론\n91 공모주청약자금대출\n92 매입자금\n93 미수론서비스\n94 대여")
    ...
    splt_buy_attr_name: str = Field(title="분할매수속성명", max_length=60, description="정규장 종료 주문 시에는 '정규장 종료', 시간 입력 시에는 from ~ to 시간 표시됨")
...
class StockBalanceItem1(BaseModel):
    ...
    frcr_evlu_pfls_amt: str = Field(
        title="외화평가손익금액",
        max_length=30,
        description="해당 종목의 매입금액과 평가금액의 외회기준 비교 손익"
    )
    evlu_pfls_rt: str = Field(
        title="평가손익율",
        max_length=10,
        description="해당 종목의 평가손익을 기준으로 한 수익률"
    )
    pchs_avg_pric: str = Field(
        title="매입평균가격",
        max_length=23,
        description="해당 종목의 매수 평균 단가"
    )
    ovrs_cblc_qty: str = Field(title="해외잔고수량", max_length=19)
    ord_psbl_qty: str = Field(
        title="주문가능수량",
        max_length=10,
        description="매도 가능한 주문 수량"
    )
    frcr_pchs_amt1: str = Field(
        title="외화매입금액1",
        max_length=23,
        description="해당 종목의 외화 기준 매입금액"
    )
    ovrs_stck_evlu_amt: str = Field(
        title="해외주식평가금액",
        max_length=32,
        description="해당 종목의 외화 기준 평가금액"
    )
    now_pric2: str = Field(
        title="현재가격2",
        max_length=25,
        description="해당 종목의 현재가"
    )
...
class StockConclusionHistoryItem(BaseModel):
    ...
    ord_gno_brno: str = Field(title="주문채번지점번호", max_length=5, description="계좌 개설 시 관리점으로 선택한 영업점의 고유번호")
    odno: str = Field(title="주문번호", max_length=10, description="접수한 주문의 일련번호 ※ 정정취소주문 시, 해당 값 odno(주문번호) 넣어서 사용")
    orgn_odno: str = Field(title="원주문번호", max_length=10, description="정정 또는 취소 대상 주문의 일련번호")
    sll_buy_dvsn_cd: str = Field(title="매도매수구분코드", max_length=2, description="01 : 매도  02 : 매수")
    ...
    usa_amk_exts_rqst_yn: str = Field(title="미국애프터마켓연장신청여부", max_length=1, description="Y/N")
    splt_buy_attr_name: str = Field(title="분할매수/매도속성명", max_length=60, description="정규장 종료 주문 시에는 '정규장 종료', 시간 입력 시에는 from ~ to 시간 표시")
...
class CurrentBalanceByConclusionItem1(BaseModel):
    ...
    evlu_pfls_amt2: str = Field(title="평가손익금액2", max_length=31, description="해당 종목의 매입금액과 평가금액의 외회기준 비교 손익")
    evlu_pfls_rt1: str = Field(title="평가손익율1", max_length=32, description="해당 종목의 평가손익을 기준으로 한 수익률")
    pdno: str = Field(title="상품번호", max_length=12, description="종목코드")
    bass_exrt: str = Field(title="기준환율", max_length=31, description="원화 평가 시 적용 환율")
    buy_crcy_cd: str = Field(title="매수통화코드", max_length=3, description="USD : 미국달러\nHKD : 홍콩달러\nCNY : 중국위안화\nJPY : 일본엔화\nVND : 베트남동")
    ...
    thdt_buy_ccld_frcr_amt: str = Field(title="당일매수체결외화금액", max_length=30, description="당일 매수 외화금액 (Type: Object X String O)")
...
class CurrentBalanceByConclusionItem2(BaseModel):
    ...
    frcr_buy_amt_smtl: str = Field(title="외화매수금액합계", max_length=29, description="해당 통화로 매수한 종목 전체의 매수금액")
    frcr_sll_amt_smtl: str = Field(title="외화매도금액합계", max_length=29, description="해당 통화로 매도한 종목 전체의 매수금액")
    ...
    pchs_amt_smtl: str = Field(title="매입금액합계", max_length=19, description="해외유가증권 매수금액의 원화 환산 금액")
    evlu_amt_smtl: str = Field(title="평가금액합계", max_length=19, description="해외유가증권 평가금액의 원화 환산 금액")
    evlu_pfls_amt_smtl: str = Field(title="평가손익금액합계", max_length=19, description="해외유가증권 평가손익의 원화 환산 금액")
...
class ReserveOrdersItem(BaseModel):
    ...
    splt_buy_attr_name: Optional[str] = Field(title="분할매수속성명", max_length=60, description="정규장 종료 주문 시에는 '정규장 종료', 시간 입력 시에는 from ~ to 시간 표시")
...
class PeriodProfitLossItem2(BaseModel):
    stck_sll_amt_smtl: str = Field(title="주식매도금액합계", max_length=184, description="WCRC_FRCR_DVSN_CD(원화외화구분코드)가 01(외화)이고 OVRS_EXCG_CD(해외거래소코드)가 공란(전체)인 경우 출력값 무시")
    stck_buy_amt_smtl: str = Field(title="주식매수금액합계", max_length=184, description="WCRC_FRCR_DVSN_CD(원화외화구분코드)가 01(외화)이고 OVRS_EXCG_CD(해외거래소코드)가 공란(전체)인 경우 출력값 무시")
    smtl_fee1: str = Field(title="합계수수료1", max_length=138, description="WCRC_FRCR_DVSN_CD(원화외화구분코드)가 01(외화)이고 OVRS_EXCG_CD(해외거래소코드)가 공란(전체)인 경우 출력값 무시")
    excc_dfrm_amt: str = Field(title="정산지급금액", max_length=205, description="WCRC_FRCR_DVSN_CD(원화외화구분코드)가 01(외화)이고 OVRS_EXCG_CD(해외거래소코드)가 공란(전체)인 경우 출력값 무시")
    ovrs_rlzt_pfls_tot_amt: str = Field(title="해외실현손익총금액", max_length=145, description="WCRC_FRCR_DVSN_CD(원화외화구분코드)가 01(외화)이고 OVRS_EXCG_CD(해외거래소코드)가 공란(전체)인 경우 출력값 무시")
...
class MarginAggregateItem(BaseModel):
    ...
    frcr_ord_psbl_amt1: str = Field(
        title="외화주문가능금액",
        max_length=186,
        description="원화주문가능환산금액"
    )
