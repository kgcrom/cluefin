...
class StockQuoteCurrentItem(BaseModel):
    krx_fwdg_ord_orgno: str = Field(alias="KRX_FWDG_ORD_ORGNO", description="거래소코드")
    odno: str = Field(alias="ODNO", description="주문번호")
    ord_tmd: str = Field(alias="ORD_TMD", description="주문시간")
...
class StockQuoteCorrectionCancellableQtyItem(BaseModel):
    ord_gno_brno: str = Field(title="주문채번지점번호", description="주문시 한국투자증권 시스템에서 지정된 영업점코드")
    odno: str = Field(title="주문번호", description="주문시 한국투자증권 시스템에서 채번된 주문번호")
    orgn_odno: str = Field(title="원주문번호", description="정정/취소주문 인경우 원주문번호")
    ord_dvsn_name: str = Field(title="주문구분명")
    pdno: str = Field(title="상품번호", description="종목번호(뒤 6자리만 해당)")
    prdt_name: str = Field(title="상품명", description="종목명")
    rvse_cncl_dvsn_name: str = Field(title="정정취소구분명", description="정정 또는 취소 여부 표시")
    ord_qty: str = Field(title="주문수량")
    ord_unpr: str = Field(title="주문단가", description="1주당 주문가격")
    ord_tmd: str = Field(title="주문시각", description="주문시각(시분초HHMMSS)")
    tot_ccld_qty: str = Field(title="총체결수량", description="주문 수량 중 체결된 수량")
    tot_ccld_amt: str = Field(title="총체결금액", description="주문금액 중 체결금액")
    psbl_qty: str = Field(title="가능수량", description="정정/취소 주문 가능 수량")
    sll_buy_dvsn_cd: str = Field(title="매도매수구분코드", description="01 : 매도 / 02 : 매수")
    ord_dvsn_cd: str = Field(title="주문구분코드")
    mgco_aptm_odno: str = Field(title="운용사지정주문번호")
    excg_dvsn_cd: str = Field(title="거래소구분코드")
    excg_id_dvsn_cd: str = Field(title="거래소ID구분코드")
    excg_id_dvsn_name: str = Field(title="거래소ID구분명")
    stpm_cndt_pric: str = Field(title="스톱지정가조건가격")
    stpm_efct_occr_yn: str = Field(title="스톱지정가효력발생여부", description="Y 또는 N")
...
class StockDailySeparateConclusionItem1(BaseModel):
    ord_dt: str = Field(title="주문일자", description="주문일자(YYYYMMDD)")
    ord_gno_brno: str = Field(title="주문채번지점번호", description="주문시 한국투자증권 시스템에서 지정된 영업점코드")
    odno: str = Field(title="주문번호", description="주문시 한국투자증권 시스템에서 채번된 주문번호")
    orgn_odno: str = Field(title="원주문번호", description="정정/취소주문 인경우 원주문번호")
    ord_dvsn_name: str = Field(title="주문구분명")
    ...
    ord_tmd: str = Field(title="주문시각", description="주문시각(시분초HHMMSS)")
    tot_ccld_qty: str = Field(title="총체결수량", description="주문 수량 중 체결된 수량")
    avg_prvs: str = Field(title="평균가", description="체결된 가격의 평균가")
    cncl_yn: str = Field(title="취소여부", description="Y 또는 N")
    tot_ccld_amt: str = Field(title="총체결금액", description="주문금액 중 체결금액")
    loan_dt: str = Field(title="대출일자", description="신용주문인 경우 대출일자(YYYYMMDD)")
    ordr_empno: str = Field(title="주문자사번", description="주문한 직원의 사번")
    ord_dvsn_cd: str = Field(title="주문구분코드")
    cnc_cfrm_qty: str = Field(title="취소확인수량", description="취소주문시 취소가 확인된 수량")
    rmn_qty: str = Field(title="잔여수량", description="주문수량 중 체결 및 취소되지 않은 잔여수량")
    rjct_qty: str = Field(title="거부수량", description="주문수량 중 거부된 수량")
    ccld_cndt_name: str = Field(title="체결조건명", description="지정가, 시장가 등 체결조건명")
    inqr_ip_addr: str = Field(title="조회IP주소", description="주문을 요청한 PC의 IP주소")
    cpbc_ordp_ord_rcit_dvsn_cd: str = Field(title="전산주문표주문접수구분코드")
    cpbc_ordp_infm_mthd_dvsn_cd: str = Field(title="전산주문표통보방법구분코드")
    infm_tmd: str = Field(title="통보시각", description="주문접수 통보시각(시분초HHMMSS)")
    ctac_tlno: str = Field(title="연락전화번호", description="주문시 연락가능한 전화번호")
    prdt_type_cd: str = Field(title="상품유형코드", description="상품유형코드")
    excg_dvsn_cd: str = Field(title="거래소구분코드")
    cpbc_ordp_mtrl_dvsn_cd: str = Field(title="전산주문표자료구분코드")
    ord_orgno: str = Field(title="주문조직번호")
    rsvn_ord_end_dt: str = Field(title="예약주문종료일자", description="예약주문인 경우 예약종료일자(YYYYMMDD)")
    excg_id_dvsn_cd: str = Field(title="거래소ID구분코드")
    stpm_cndt_pric: str = Field(title="스톱지정가조건가격")
    stpm_efct_occr_dtmd: str = Field(
        title="스톱지정가효력발생상세시각", description="스톱지정가효력발생상세시각(시분초HHMMSS)"
    )


class StockDailySeparateConclusionItem2(BaseModel):
    tot_ord_qty: str = Field(title="총주문수량", description="조회기간내 총 주문수량")
    tot_ccld_qty: str = Field(title="총체결수량", description="조회기간내 총 체결수량")
    tot_ccld_amt: str = Field(title="총체결금액", description="조회기간내 총 체결금액")
    pchs_avg_pric: str = Field(title="매입평균가격", description="조회기간내 매입평균가격")
    prsm_tlex_smtl: str = Field(title="추정제비용합계", description="조회기간내 추정제비용합계")
...
class StockBalanceItem1(BaseModel):
    pdno: str = Field(title="상품번호", description="종목번호(뒤 6자리만 해당)")
    prdt_name: str = Field(title="상품명", description="종목명")
    trad_dvsn_name: str = Field(title="매매구분명", description="매수매도구분")
    bfdy_buy_qty: str = Field(title="전일매수수량")
    bfdy_sll_qty: str = Field(title="전일매도수량")
    thdt_buyqty: str = Field(title="금일매수수량")
    thdt_sll_qty: str = Field(title="금일매도수량")
    hldg_qty: str = Field(title="보유수량")
    ord_psbl_qty: str = Field(title="주문가능수량")
    pchs_avg_pric: str = Field(title="매입평균가격", description="매입금액 / 보유수량")
    pchs_amt: str = Field(title="매입금액")
    prpr: str = Field(title="현재가")
    evlu_amt: str = Field(title="평가금액")
    evlu_pfls_amt: str = Field(title="평가손익금액", description="평가금액 - 매입금액")
    evlu_pfls_rt: str = Field(title="평가손익율")
    evlu_erng_rt: str = Field(title="평가수익율", description="미사용항목(0으로 출력)")
    loan_dt: str = Field(title="대출일자", description="INQR_DVSN(조회구분)을 01(대출일별)로 설정해야 값이 나옴")
    loan_amt: str = Field(title="대출금액")
    stln_slng_chgs: str = Field(title="대주매각대금")
    expd_dt: str = Field(title="만기일자")
    fltt_rt: str = Field(title="등락율")
    bfdy_cprs_icdc: str = Field(title="전일대비증감")
    item_mgna_rt_name: str = Field(title="종목증거금율명")
    grta_rt_name: str = Field(title="보증금율명")
    sbst_pric: str = Field(
        title="대용가격", description="증권매매의 위탁보증금으로서 현금 대신에 사용되는 유가증권 가격"
    )
    stck_loan_unpr: str = Field(title="주식대출단가")


class StockBalanceItem2(BaseModel):
    dnca_tot_amt: str = Field(title="예수금총금액", description="예수금")
    nxdy_excc_amt: str = Field(title="익일정산금액", description="D+1 예수금")
    prvs_rcdl_excc_amt: str = Field(title="가수도정산금액", description="D+2 예수금")
    cma_evlu_amt: str = Field(title="CMA평가금액")
    bfdy_buy_amt: str = Field(title="전일매수금액")
    thdt_buy_amt: str = Field(title="금일매수금액")
    nxdy_auto_rdpt_amt: str = Field(title="익일자동상환금액")
    bfdy_sll_amt: str = Field(title="전일매도금액")
    thdt_sll_amt: str = Field(title="금일매도금액")
    d2_auto_rdpt_amt: str = Field(title="D+2자동상환금액")
    bfdy_tlex_amt: str = Field(title="전일제비용금액")
    thdt_tlex_amt: str = Field(title="금일제비용금액")
    tot_loan_amt: str = Field(title="총대출금액")
    scts_evlu_amt: str = Field(title="유가평가금액")
    tot_evlu_amt: str = Field(title="총평가금액", description="유가증권 평가금액 합계금액 + D+2 예수금")
    nass_amt: str = Field(title="순자산금액")
    ...
    pchs_amt_smtl_amt: str = Field(title="매입금액합계금액")
    evlu_amt_smtl_amt: str = Field(title="평가금액합계금액", description="유가증권 평가금액 합계금액")
    evlu_pfls_smtl_amt: str = Field(title="평가손익합계금액")
    tot_stln_slng_chgs: str = Field(title="총대주매각대금")
    bfdy_tot_asst_evlu_amt: str = Field(title="전일총자산평가금액")
    asst_icdc_amt: str = Field(title="자산증감액")
    asst_icdc_erng_rt: str = Field(title="자산증감수익율", description="데이터 미제공")
...
class BuyTradableInquiryItem(BaseModel):
    ord_psbl_cash: str = Field(title="주문가능현금")
    ord_psbl_sbst: str = Field(title="주문가능대용")
    ruse_psbl_amt: str = Field(title="재사용가능금액")
    fund_rpch_chgs: str = Field(title="펀드환매대금")
    psbl_qty_calc_unpr: str = Field(title="가능수량계산단가")
    nrcvb_buy_amt: str = Field(title="미수없는매수금액")
    nrcvb_buy_qty: str = Field(title="미수없는매수수량")
    max_buy_amt: str = Field(title="최대매수금액")
    max_buy_qty: str = Field(title="최대매수수량")
    cma_evlu_amt: str = Field(title="CMA평가금액")
    ovrs_re_use_amt_wcrc: str = Field(title="해외재사용금액원화")
    ord_psbl_frcr_amt_wcrc: str = Field(title="주문가능외화금액원화")
...
class SellTradableInquiryItem(BaseModel):
    pdno: str = Field(title="상품번호", description="종목번호(뒤 6자리만 해당)")
    prdt_name: str = Field(title="상품명", description="종목명")
    buy_qty: str = Field(title="매수수량")
    sll_qty: str = Field(title="매도수량")
    cblc_qty: str = Field(title="잔고수량")
    nsvg_qty: str = Field(title="비저축수량")
    ord_psbl_qty: str = Field(title="주문가능수량")
    pchs_avg_pric: str = Field(title="매입평균가격", description="매입금액 / 보유수량")
    pchs_amt: str = Field(title="매입금액")
    now_pric: str = Field(title="현재가")
    evlu_amt: str = Field(title="평가금액")
    evlu_pfls_amt: str = Field(title="평가손익금액", description="평가금액 - 매입금액")
    evlu_pfls_rt: str = Field(title="평가손익율")
...
class StockReserveQuoteItem(BaseModel):
    rsvn_ord_seq: Optional[str] = Field(title="예약주문순번")
...
class StockReserveQuoteCorrectionItem(BaseModel):
    nrml_prcs_yn: str = Field(title="정상처리여부")
...
class StockReserveQuoteInquiryItem(BaseModel):
    rsvn_ord_seq: str = Field(title="예약주문 순번")
    rsvn_ord_ord_dt: str = Field(title="예약주문주문일자")
    rsvn_ord_rcit_dt: str = Field(title="예약주문접수일자")
    pdno: str = Field(title="상품번호")
    ord_dvsn_cd: str = Field(title="주문구분코드")
    ord_rsvn_qty: str = Field(title="주문예약수량")
    tot_ccld_qty: str = Field(title="총체결수량")
    cncl_ord_dt: str = Field(title="취소주문일자")
    ord_tmd: str = Field(title="주문시각")
    ctac_tlno: str = Field(title="연락전화번호")
    rjct_rson2: str = Field(title="거부사유2")
    odno: str = Field(title="주문번호")
    rsvn_ord_rcit_tmd: str = Field(title="예약주문접수시각")
    kor_item_shtn_name: str = Field(title="한글종목단축명")
    sll_buy_dvsn_cd: str = Field(title="매도매수구분코드")
    ord_rsvn_unpr: str = Field(title="주문예약단가")
    tot_ccld_amt: str = Field(title="총체결금액")
    loan_dt: str = Field(title="대출일자")
    cncl_rcit_tmd: str = Field(title="취소접수시각")
    prcs_rslt: str = Field(title="처리결과")
    ord_dvsn_name: str = Field(title="주문구분명")
    tmnl_mdia_kind_cd: str = Field(title="단말매체종류코드")
    rsvn_end_dt: str = Field(title="예약종료일자")
...
class PensionConclusionBalanceItem1(BaseModel):
    cblc_dvsn: str = Field(title="잔고구분")
    cblc_dvsn_name: str = Field(title="잔고구분명")
    pdno: str = Field(title="상품번호")
    prdt_name: str = Field(title="상품명")
    hldg_qty: str = Field(title="보유수량")
    slpsb_qty: str = Field(title="매도가능수량")
    pchs_avg_pric: str = Field(title="매입평균가격")
    evlu_pfls_amt: str = Field(title="평가손익금액")
    evlu_pfls_rt: str = Field(title="평가손익율")
    prpr: str = Field(title="현재가")
    evlu_amt: str = Field(title="평가금액")
    pchs_amt: str = Field(title="매입금액")
    cblc_weit: str = Field(title="잔고비중")


class PensionConclusionBalanceItem2(BaseModel):
    pchs_amt_smtl_amt: str = Field(title="매입금액합계금액")
    evlu_amt_smtl_amt: str = Field(title="평가금액합계금액")
    evlu_pfls_smtl_amt: str = Field(title="평가손익합계금액")
    trad_pfls_smtl: str = Field(title="매매손익합계")
    thdt_tot_pfls_amt: str = Field(title="당일총손익금액")
    pftrt: str = Field(title="수익률")
...
class PensionNotConclusionHistoryItem(BaseModel):
    ord_gno_brno: str = Field(alias="ord_gno_brno", title="주문채번지점번호")
    sll_buy_dvsn_cd: str = Field(alias="sll_buy_dvsn_cd", title="매도매수구분코드")
    trad_dvsn_name: str = Field(alias="trad_dvsn_name", title="매매구분명")
    odno: str = Field(alias="odno", title="주문번호")
    pdno: str = Field(alias="pdno", title="상품번호")
    prdt_name: str = Field(alias="prdt_name", title="상품명")
    ord_unpr: str = Field(alias="ord_unpr", title="주문단가")
    ord_qty: str = Field(alias="ord_qty", title="주문수량")
    tot_ccld_qty: str = Field(alias="tot_ccld_qty", title="총체결수량")
    nccs_qty: str = Field(alias="nccs_qty", title="미체결수량")
    ord_dvsn_cd: str = Field(alias="ord_dvsn_cd", title="주문구분코드")
    ord_dvsn_name: str = Field(alias="ord_dvsn_name", title="주문구분명")
    orgn_odno: str = Field(alias="orgn_odno", title="원주문번호")
    ord_tmd: str = Field(alias="ord_tmd", title="주문시각")
    objt_cust_dvsn_name: str = Field(alias="objt_cust_dvsn_name", title="대상고객구분명")
    pchs_avg_pric: str = Field(alias="pchs_avg_pric", title="매입평균가격")
    stpm_cndt_pric: str = Field(alias="stpm_cndt_pric", title="스톱지정가조건가격")
    stpm_efct_occr_dtmd: str = Field(alias="stpm_efct_occr_dtmd", title="스톱지정가효력발생상세시각")
    stpm_efct_occr_yn: str = Field(alias="stpm_efct_occr_yn", title="스톱지정가효력발생여부")
    excg_id_dvsn_cd: str = Field(alias="excg_id_dvsn_cd", title="거래소ID구분코드")
...
class PensionBuyTradableInquiryItem(BaseModel):
    ord_psbl_cash: str = Field(alias="ord_psbl_cash", title="주문가능현금")
    ruse_psbl_amt: str = Field(alias="ruse_psbl_amt", title="재사용가능금액")
    psbl_qty_calc_unpr: str = Field(alias="psbl_qty_calc_unpr", title="가능수량계산단가")
    max_buy_amt: str = Field(alias="max_buy_amt", title="최대매수금액")
    max_buy_qty: str = Field(alias="max_buy_qty", title="최대매수수량")
...
class PensionReserveDepositInquiryItem(BaseModel):
    dnca_tota: str = Field(alias="dnca_tota", title="예수금총액")
    nxdy_excc_amt: str = Field(alias="nxdy_excc_amt", title="익일정산액")
    nxdy_sttl_amt: str = Field(alias="nxdy_sttl_amt", title="익일결제금액")
    nx2_day_sttl_amt: str = Field(alias="nx2_day_sttl_amt", title="익일결제금액")
...
class PensionBalanceInquiryItem1(BaseModel):
    cblc_dvsn_name: str = Field(alias="cblc_dvsn_name", title="잔고구분명")
    prdt_name: str = Field(alias="prdt_name", title="상품명")
    pdno: str = Field(alias="pdno", title="상품번호")
    item_dvsn_name: str = Field(alias="item_dvsn_name", title="종목구분명")
    thdt_buyqty: str = Field(alias="thdt_buyqty", title="금일매수수량")
    thdt_sll_qty: str = Field(alias="thdt_sll_qty", title="금일매도수량")
    hldg_qty: str = Field(alias="hldg_qty", title="보유수량")
    ord_psbl_qty: str = Field(alias="ord_psbl_qty", title="주문가능수량")
    pchs_avg_pric: str = Field(alias="pchs_avg_pric", title="매입평균가격")
    pchs_amt: str = Field(alias="pchs_amt", title="매입금액")
    prpr: str = Field(alias="prpr", title="현재가")
    evlu_amt: str = Field(alias="evlu_amt", title="평가금액")
    evlu_pfls_amt: str = Field(alias="evlu_pfls_amt", title="평가손익금액")
    evlu_erng_rt: str = Field(alias="evlu_erng_rt", title="평가수익율")


class PensionBalanceInquiryItem2(BaseModel):
    dnca_tot_amt: str = Field(alias="dnca_tot_amt", title="예수금총금액")
    nxdy_excc_amt: str = Field(alias="nxdy_excc_amt", title="익일정산금액")
    prvs_rcdl_excc_amt: str = Field(alias="prvs_rcdl_excc_amt", title="가수도정산금액")
    thdt_buy_amt: str = Field(alias="thdt_buy_amt", title="금일매수금액")
    thdt_sll_amt: str = Field(alias="thdt_sll_amt", title="금일매도금액")
    thdt_tlex_amt: str = Field(alias="thdt_tlex_amt", title="금일제비용금액")
    scts_evlu_amt: str = Field(alias="scts_evlu_amt", title="유가평가금액")
    tot_evlu_amt: str = Field(alias="tot_evlu_amt", title="총평가금액")
...
class StockBalanceLossProfitItem1(BaseModel):
    ...
    prdt_name: str = Field(alias="prdt_name", title="상품명")
    trad_dvsn_name: str = Field(alias="trad_dvsn_name", title="매매구분명")
    bfdy_buy_qty: str = Field(alias="bfdy_buy_qty", title="전일매수수량")
    bfdy_sll_qty: str = Field(alias="bfdy_sll_qty", title="전일매도수량")
    thdt_buyqty: str = Field(alias="thdt_buyqty", title="금일매수수량")
    thdt_sll_qty: str = Field(alias="thdt_sll_qty", title="금일매도수량")
    hldg_qty: str = Field(alias="hldg_qty", title="보유수량")
    ord_psbl_qty: str = Field(alias="ord_psbl_qty", title="주문가능수량")
    ...
    evlu_amt: str = Field(alias="evlu_amt", title="평가금액")
    evlu_pfls_amt: str = Field(alias="evlu_pfls_amt", title="평가손익금액")
    evlu_pfls_rt: str = Field(alias="evlu_pfls_rt", title="평가손익율")
    evlu_erng_rt: str = Field(alias="evlu_erng_rt", title="평가수익율")
    loan_dt: str = Field(alias="loan_dt", title="대출일자")
    loan_amt: str = Field(alias="loan_amt", title="대출금액")
    stln_slng_chgs: str = Field(alias="stln_slng_chgs", title="대주매각대금")
    expd_dt: str = Field(alias="expd_dt", title="만기일자")
    stck_loan_unpr: str = Field(alias="stck_loan_unpr", title="주식대출단가")
    bfdy_cprs_icdc: str = Field(alias="bfdy_cprs_icdc", title="전일대비증감")
    fltt_rt: str = Field(alias="fltt_rt", title="등락율")
...
class StockBalanceLossProfitItem2(BaseModel):
    ...
    nxdy_excc_amt: str = Field(alias="nxdy_excc_amt", title="익일정산금액")
    prvs_rcdl_excc_amt: str = Field(alias="prvs_rcdl_excc_amt", title="가수도정산금액")
    cma_evlu_amt: str = Field(alias="cma_evlu_amt", title="CMA평가금액")
    bfdy_buy_amt: str = Field(alias="bfdy_buy_amt", title="전일매수금액")
    thdt_buy_amt: str = Field(alias="thdt_buy_amt", title="금일매수금액")
    nxdy_auto_rdpt_amt: str = Field(alias="nxdy_auto_rdpt_amt", title="익일자동상환금액")
    bfdy_sll_amt: str = Field(alias="bfdy_sll_amt", title="전일매도금액")
    thdt_sll_amt: str = Field(alias="thdt_sll_amt", title="금일매도금액")
    d2_auto_rdpt_amt: str = Field(alias="d2_auto_rdpt_amt", title="D+2자동상환금액")
    bfdy_tlex_amt: str = Field(alias="bfdy_tlex_amt", title="전일제비용금액")
    thdt_tlex_amt: str = Field(alias="thdt_tlex_amt", title="금일제비용금액")
    tot_loan_amt: str = Field(alias="tot_loan_amt", title="총대출금액")
    scts_evlu_amt: str = Field(alias="scts_evlu_amt", title="유가평가금액")
    tot_evlu_amt: str = Field(alias="tot_evlu_amt", title="총평가금액")
    nass_amt: str = Field(alias="nass_amt", title="순자산금액")
    fncg_gld_auto_rdpt_yn: str = Field(alias="fncg_gld_auto_rdpt_yn", title="융자금자동상환여부")
    pchs_amt_smtl_amt: str = Field(alias="pchs_amt_smtl_amt", title="매입금액합계금액")
    evlu_amt_smtl_amt: str = Field(alias="evlu_amt_smtl_amt", title="평가금액합계금액")
    evlu_pfls_smtl_amt: str = Field(alias="evlu_pfls_smtl_amt", title="평가손익합계금액")
    tot_stln_slng_chgs: str = Field(alias="tot_stln_slng_chgs", title="총대주매각대금")
    bfdy_tot_asst_evlu_amt: str = Field(alias="bfdy_tot_asst_evlu_amt", title="전일총자산평가금액")
    asst_icdc_amt: str = Field(alias="asst_icdc_amt", title="자산증감액")
    asst_icdc_erng_rt: str = Field(alias="asst_icdc_erng_rt", title="자산증감수익율")
    rlzt_pfls: str = Field(alias="rlzt_pfls", title="실현손익")
    rlzt_erng_rt: str = Field(alias="rlzt_erng_rt", title="실현수익율")
    real_evlu_pfls: str = Field(alias="real_evlu_pfls", title="실평가손익")
    real_evlu_pfls_erng_rt: str = Field(alias="real_evlu_pfls_erng_rt", title="실평가손익수익율")
