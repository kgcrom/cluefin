...
class SectorCurrentIndexItem(BaseModel):
    ...
    prdy_tr_pbmn: str = Field(title="전일 거래 대금")
    bstp_nmix_oprc: str = Field(title="업종 지수 시가2")
    prdy_nmix_vrss_nmix_oprc: str = Field(title="전일 지수 대비 지수 시가2")
    oprc_vrss_prpr_sign: str = Field(title="시가2 대비 현재가 부호")
    bstp_nmix_oprc_prdy_ctrt: str = Field(title="업종 지수 시가2 전일 대비율")
    bstp_nmix_hgpr: str = Field(title="업종 지수 최고가")
    prdy_nmix_vrss_nmix_hgpr: str = Field(title="전일 지수 대비 지수 최고가")
    hgpr_vrss_prpr_sign: str = Field(title="최고가 대비 현재가 부호")
    bstp_nmix_hgpr_prdy_ctrt: str = Field(title="업종 지수 최고가 전일 대비율")
    bstp_nmix_lwpr: str = Field(title="업종 지수 최저가")
    prdy_clpr_vrss_lwpr: str = Field(title="전일 종가 대비 최저가")
    ...
    uplm_issu_cnt: str = Field(title="상한 종목 수")
    stnr_issu_cnt: str = Field(title="보합 종목 수")
    down_issu_cnt: str = Field(title="하락 종목 수")
    lslm_issu_cnt: str = Field(title="하한 종목 수")
    dryy_bstp_nmix_hgpr: str = Field(title="연중업종지수최고가")
    dryy_hgpr_vrss_prpr_rate: str = Field(title="연중 최고가 대비 현재가 비율")
    dryy_bstp_nmix_hgpr_date: str = Field(title="연중업종지수최고가일자")
    dryy_bstp_nmix_lwpr: str = Field(title="연중업종지수최저가")
    dryy_lwpr_vrss_prpr_rate: str = Field(title="연중 최저가 대비 현재가 비율")
    dryy_bstp_nmix_lwpr_date: str = Field(title="연중업종지수최저가일자")
    ...
    shnu_rsqn_rate: str = Field(title="매수2 잔량 비율")
    ntby_rsqn: str = Field(title="순매수 잔량")
...
class SectorDailyIndexItem2(BaseModel):
    ...
    bstp_nmix_hgpr: str = Field(title="업종 지수 최고가")
    bstp_nmix_lwpr: str = Field(title="업종 지수 최저가")
    acml_vol_rlim: str = Field(title="누적 거래량 비중")
    acml_vol: str = Field(title="누적 거래량")
    acml_tr_pbmn: str = Field(title="누적 거래 대금")
    invt_new_psdg: str = Field(title="투자 신 심리도")
...
class SectorMinuteInquiryItem1(BaseModel):
    ...
    bstp_nmix_hgpr: str = Field(title="업종 지수 최고가")
    bstp_nmix_lwpr: str = Field(title="업종 지수 최저가")
    futs_prdy_oprc: str = Field(title="선물 전일 시가")
    futs_prdy_hgpr: str = Field(title="선물 전일 최고가")
    futs_prdy_lwpr: str = Field(title="선물 전일 최저가")
...
class SectorAllQuoteByCategoryItem2(BaseModel):
    ...
    acml_tr_pbmn: str = Field(title="누적 거래 대금")
    acml_vol_rlim: str = Field(title="누적 거래량 비중")
    acml_tr_pbmn_rlim: str = Field(title="누적 거래 대금 비중")
...
class ExpectedIndexTrendItem(BaseModel):
    stck_cntg_hour: str = Field(title="주식 단축 종목코드")
    bstp_nmix_prpr: str = Field(title="HTS 한글 종목명")
    prdy_vrss_sign: str = Field(title="주식 현재가")
    bstp_nmix_prdy_vrss: str = Field(title="전일 대비")
    prdy_ctrt: str = Field(title="전일 대비 부호")
    acml_vol: str = Field(title="전일 대비율")
    acml_tr_pbmn: str = Field(title="기준가 대비 현재가")
...
class ExpectedIndexAllItem2(BaseModel):
    ...
    bstp_nmix_prdy_ctrt: str = Field(title="업종 지수 전일 대비율")
    acml_vol: str = Field(title="누적 거래량")
    nmix_sdpr: str = Field(title="지수 기준가")
    ascn_issu_cnt: str = Field(title="상승 종목 수")
    stnr_issu_cnt: str = Field(title="보합 종목 수")
    down_issu_cnt: str = Field(title="하락 종목 수")
...
class VolatilityInterruptionStatusItem(BaseModel):
    ...
    vi_cls_code: str = Field(title="VI발동상태", description="Y: 발동 / N: 해제")
    bsop_date: str = Field(title="영업 일자")
    cntg_vi_hour: str = Field(title="VI발동시간", description="VI발동시간")
    vi_cncl_hour: str = Field(title="VI해제시간", description="VI해제시간")
    vi_kind_code: str = Field(title="VI종류코드", description="1:정적 2:동적 3:정적&동적")
    vi_prc: str = Field(title="VI발동가격")
    vi_stnd_prc: str = Field(title="정적VI발동기준가격")
    vi_dprt: str = Field(title="정적VI발동괴리율", description="%")
    vi_dmc_stnd_prc: str = Field(title="동적VI발동기준가격")
    vi_dmc_dprt: str = Field(title="동적VI발동괴리율", description="%")
    vi_count: str = Field(title="VI발동횟수")
...
class InterestRateSummaryItem1(BaseModel):
    bcdt_code: str = Field(title="자료코드")
    hts_kor_isnm: str = Field(title="HTS한글종목명")
    bond_mnrt_prpr: str = Field(title="채권금리현재가")
    prdy_vrss_sign: str = Field(title="전일대비부호")
    bond_mnrt_prdy_vrss: str = Field(title="채권금리전일대비")
...
class InterestRateSummaryItem2(BaseModel):
    ...
    prdy_vrss_sign: str = Field(title="전일대비부호")
    bond_mnrt_prdy_vrss: str = Field(title="채권금리전일대비")
    bstp_nmix_prdy_ctrt: str = Field(title="업종지수전일대비율")
    stck_bsop_date: Optional[str] = Field(default="", title="주식영업일자")
...
class MarketAnnouncementScheduleItem(BaseModel):
    cntt_usiq_srno: str = Field(title="내용 조회용 일련번호")
    ...
    hts_pbnt_titl_cntt: str = Field(title="HTS 공시 제목 내용")
    ...
    iscd1: str = Field(title="종목 코드1")
    iscd2: str = Field(title="종목 코드2")
...
class HolidayInquiryItem(BaseModel):
    bass_dt: str = Field(title="기준일자", description="기준일자(YYYYMMDD)")
    wday_dvsn_cd: str = Field(
        title="요일구분코드",
        description="01:일요일, 02:월요일, 03:화요일, 04:수요일, 05:목요일, 06:금요일, 07:토요일",
    )
    bzdy_yn: str = Field(title="영업일여부", description="Y/N 금융기관이 업무를 하는 날")
    tr_day_yn: str = Field(title="거래일여부", description="Y/N 증권 업무가 가능한 날(입출금, 이체 등의 업무 포함)")
    opnd_yn: str = Field(
        title="개장일여부",
        description="Y/N 주식시장이 개장되는 날 * 주문을 넣고자 할 경우 개장일여부(opnd_yn)를 사용",
    )
    sttl_day_yn: str = Field(
        title="결제일여부", description="Y/N 주식 거래에서 실제로 주식을 인수하고 돈을 지불하는 날"
    )
...
class FuturesBusinessDayInquiryItem(BaseModel):
    date1: str = Field(title="영업일1")
    date2: str = Field(title="영업일2")
    date3: str = Field(title="영업일3", description="영업일 당일")
    date4: str = Field(title="영업일4")
    date5: str = Field(title="영업일5")
    today: str = Field(title="오늘일자")
    time: str = Field(title="현재시간")
    s_time: str = Field(title="장시작시간")
    e_time: str = Field(title="장마감시간")
