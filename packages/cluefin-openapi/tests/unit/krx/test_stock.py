...
def test_get_kospi_base_info(client: Client):
    ...
    with requests_mock.Mocker() as m:
        ...
        assert len(response.body.data) == 3
        assert response.body.data[0].issue_name == "ESR켄달스퀘어리츠보통주"
        assert response.body.data[0].issue_abbreviation == "ESR켄달스퀘어리츠"
...
def test_get_kosdaq_base_info(client: Client):
    ...
    with requests_mock.Mocker() as m:
        ...
        assert len(response.body.data) == 3
        assert response.body.data[0].issue_name == "SBI핀테크솔루션즈"
...
def test_get_konex_base_info(client: Client):
    ...
    with requests_mock.Mocker() as m:
        ...
        assert len(response.body.data) == 3
        assert response.body.data[0].issue_name == "길교이앤씨"
