...
class TestCreditSell:
    def test_sends_input_envelope_and_parses_output(self, client):
        with requests_mock.Mocker() as m:
            m.post(f"{BASE_PROD}/krstock/order/v1/creditSell", json=CASH_BUY_BODY)
...
class TestCancel:
    def test_sends_input_envelope_and_parses_output(self, client):
        with requests_mock.Mocker() as m:
            m.post(f"{BASE_PROD}/krstock/order/v1/cancel", json=CANCEL_BODY)
    ...
    def test_parses_body_without_output_block(self, client):
        # Output_N 블록은 데이터가 있을 때만 내려온다.
        with requests_mock.Mocker() as m:
            m.post(f"{BASE_PROD}/krstock/order/v1/cancel", json={"rsp_cd": "00000", "rsp_msg": "ok"})
            response = client.krstock_order.cancel(
                act_no="50051036881",
                org_mkt_orr_no=12345,
                all_pat_dit_cd="1",
