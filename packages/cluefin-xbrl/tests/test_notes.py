...
_IDENTIFY_NOTE_ROLE_CASES = [
    pytest.param("http://dart.fss.or.kr/role/ifrs/ias_19_role-D834480", ("D834480", True), id="consolidated_note"),
    pytest.param("http://dart.fss.or.kr/role/ifrs/ias_19_role-D834485", ("D834485", False), id="separate_note"),
    pytest.param("http://dart.fss.or.kr/role/ifrs/dart_2024-06-30_role-D210000", None, id="statement_role_is_not_note"),
    pytest.param("http://dart.fss.or.kr/role/ifrs/dart-gcd_2024-06-30_role-D999001", None, id="doc_role_is_not_note"),
    pytest.param("http://example.com/role/SomeOtherRole", None, id="generic_role_is_not_note"),
]


class TestIdentifyNoteRole:
    @pytest.mark.parametrize("linkrole, expected", _IDENTIFY_NOTE_ROLE_CASES)
    def test_identify_note_role(self, linkrole, expected):
        assert _identify_note_role(linkrole) == expected


def _make_doc(
    dart_role_bs_consolidated: str,
    ...
    taxonomy = TaxonomyInfo(
        ...
        presentation_trees={note_role: [root_node], dart_role_bs_consolidated: [stmt_node]},
    )
...
def _make_obligation_fact(period: XbrlPeriod, make_xbrl_fact) -> XbrlFact:
    return make_xbrl_fact(
        concept_local_name="DefinedBenefitObligationAtPresentValue",
        concept_qname="ifrs-full:DefinedBenefitObligationAtPresentValue",
        unit="iso4217:KRW",
        period=period,
...
class TestExtractNotes:
    ...
    def test_extracts_only_note_roles(self, dart_role_bs_consolidated):
        notes = extract_notes(_make_doc(dart_role_bs_consolidated))
        # D834480 주석만, 재무상태표 D210000은 제외
        assert list(notes.notes.keys()) == ["D834480"]

    def test_note_section_metadata(self, dart_role_bs_consolidated):
        section = extract_notes(_make_doc(dart_role_bs_consolidated)).notes["D834480"]
        assert section.role_code == "D834480"
        assert section.is_consolidated is True
        assert section.title == "종업원급여에 대한 공시"

    def test_dimensions_preserved(self, dart_role_bs_consolidated):
        section = extract_notes(_make_doc(dart_role_bs_consolidated)).notes["D834480"]
    ...
    def test_abstract_node_has_no_value(self, dart_role_bs_consolidated):
        section = extract_notes(_make_doc(dart_role_bs_consolidated)).notes["D834480"]
    ...
    def test_separate_note_section(self, dart_role_bs_consolidated):
        notes = extract_notes(
            _make_doc(dart_role_bs_consolidated, note_role="http://dart.fss.or.kr/role/ifrs/ias_19_role-D834485")
        )
        section = notes.notes["D834485"]
        assert section.is_consolidated is False

    def test_title_falls_back_to_english_label(self, dart_role_bs_consolidated):
        labels = {
            "DisclosureOfDefinedBenefitPlansAbstract": ConceptLabel(
                concept_local_name="DisclosureOfDefinedBenefitPlansAbstract",
                concept_qname="ifrs-full:DisclosureOfDefinedBenefitPlansAbstract",
        ...
        section = extract_notes(_make_doc(dart_role_bs_consolidated, labels=labels)).notes["D834480"]
        assert section.title == "Disclosure of defined benefit plans"

    def test_periods_deduplicated(self, dart_role_bs_consolidated, make_xbrl_fact):
        ...
        facts = [
            _make_obligation_fact(p_2023, make_xbrl_fact),
            _make_obligation_fact(p_2023, make_xbrl_fact),
            _make_obligation_fact(p_2022, make_xbrl_fact),
        ]
        section = extract_notes(_make_doc(dart_role_bs_consolidated, facts=facts)).notes["D834480"]
        assert section.periods == [p_2023, p_2022]

    def test_consolidation_axis_filters_facts(self, consolidated_axis, dart_role_bs_consolidated, make_xbrl_fact):
        ...
        def _fact(value: str, member: str) -> XbrlFact:
            return make_xbrl_fact(
                ...
                concept_qname="ifrs-full:DefinedBenefitObligationAtPresentValue",
                value=value,
                dimensions={consolidated_axis: member, "ifrs-full:Axis": "ifrs-full:Member"},
            )
    ...
    def test_text_value_for_string_fact(self, dart_role_bs_consolidated, make_xbrl_fact):
        fact = make_xbrl_fact(
            concept_local_name="DefinedBenefitObligationAtPresentValue",
            concept_qname="ifrs-full:DefinedBenefitObligationAtPresentValue",
            value="확정급여제도 관련 서술",
